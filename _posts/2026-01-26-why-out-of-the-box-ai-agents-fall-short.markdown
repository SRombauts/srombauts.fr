...
For the past few months I have been getting real work done with AI coding agents on our
big codebase, and along the way I have collected a pile of things that genuinely help:
Skills and scripts, small repeatable workflows, and a handful of ideas about how to work
with these tools without getting burned. Keeping all of that on my own machine would be a
waste. So I am putting together a sharing session for the team: a demo of how I actually
use Cursor day to day, plus the materials behind it, so the people I work with can pick
them up instead of starting from zero. The posts that come out of this keep our
Unity-internal specifics out and stick to what applies to anyone on a real, messy,
long-lived codebase.

This first post comes before any of the tooling. It is the part we have to agree on first:
how these models actually work, so that we share a common understanding of LLMs and a
mental model of coding agents.
I also wanted it written down somewhere I can point my next team at.

## The disappointment, related by some team members

We all installed Cursor, and fired the most capable models at our repository, to ask it
to fix a bug or add a small feature.
It answers fast, it sounds confident, and the result is sometimes awesome, but often generic
or even wrong. It does not know our code, it does not know our command-line tools,
and it has never heard of our conventions.

My setting for the whole series is a big C#/.NET codebase: the Unity Version Control
codebase, with our Unity package and the Desktop application.
The agent knows C# in general just fine. It knows nothing about *our* C#, our build, or the
CLI we use all day. So out of the box it is basically a clever stranger.
...
rather than what you actually said. If you trigger compaction, that is a sign you poured in
too much irrelevant stuff to begin with, or tried to tackled too many tasks in one sessions.
...
The practical habits that follow are boring but they work. Stay in the first half of the
window. Point to the minimum set of files the task needs, not whole folders. And start a
fresh session for each new task instead of carrying one giant chat across your whole day.
...
But remember, any of us will also answer differently the same question if asked multiple
times, depending on the context etc. And each team member will deliver something different
if you give them the same task.
...
A note of optimism here: given the tools to verify their output, coding agents will iterate
and fix their own mistake quite fast, which solves most of the issue.
It costs tokens and time, thtough.

## You stopped being the coder. You are the tech lead now

This is the shift that ties it all together. The job is no longer coding, syntax and
